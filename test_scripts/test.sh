...
# Kill previous ports first (Removed sudo to prevent hanging on password prompts)
PREV_PIDS=$(lsof -ti:$PORT_RECV,$PORT_SEND)
if [ -n "$PREV_PIDS" ]; then
  kill -9 $PREV_PIDS
fi
...
sleep 10
# Fixed typo: $SENDER_ID changed to $SENDER_PID. Removed sudo.
kill -INT $RECEIVER_PID $PROXY_PID $SENDER_PID
